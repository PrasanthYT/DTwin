...
import { ChevronLeft, ChevronRight, Search, X, Check, Plus } from "lucide-react";
...
export default function AddMeds() {
    ...
    const fetchUserMedications = async () => {
      try {
        ...
        if (response.status === 200) {
          const userMeds = response.data?.user?.userDetails?.medications || [];
          setSelectedMedications(userMeds);
        }
      } catch (error) {
    ...
    // ✅ Fetch Available Medications (Filtered)
    const filterMedications = () => {
      ...
      const filteredMeds = MEDICATIONS_DATABASE.filter((med) => {
        ...
        const matchesLetter =
          !alphabetFilter || med.name.toLowerCase().startsWith(alphabetFilter.toLowerCase());
    ...
    // ✅ Add or Remove Medication from Selection
    ...
    const removeSelectedMedication = (medication) => {
      setSelectedMedications((prev) => prev.filter((m) => m.name !== medication.name));
    };
  
    // ✅ Submit Updated Medications
    ...
    return (
      <div className="h-screen bg-white flex flex-col max-w-md mx-auto p-4">
        ...
        <div className="space-y-4">
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              <div className="flex flex-wrap gap-2">
                {selectedMedications.map((medication) => (
                  <span key={medication.name} className="bg-gray-200 p-2 rounded-md flex items-center">
                    {medication.name}
                    <button onClick={() => removeSelectedMedication(medication)} className="ml-2 text-red-500">
                      <X className="h-4 w-4" />
                    </button>
                  </span>
                ))}
          ...
          <Button className="w-full bg-[#0066FF] text-white rounded-xl py-4 flex items-center justify-center gap-2" onClick={handleSubmit}>
            Add Medications <Plus className="h-5 w-5" />
          </Button>
          <Button onClick={handleback} className="w-full bg-gray-500 hover:bg-gray-500 text-white rounded-xl py-4 flex items-center justify-center gap-2">
            Close
          </Button>
        </div>
      </div>
