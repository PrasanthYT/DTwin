...
export default function HealthSymptoms() {
    ...
    const addCustomSymptom = (e) => {
        if (e.key === 'Enter' && customSymptom.trim() && !selectedSymptoms.includes(customSymptom.trim())) {
            e.preventDefault();
            setSelectedSymptoms(prev => {
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                {predefinedSymptoms.map(symptom => (
                    <button
                        ...
                        className={`px-3 py-2 rounded-full text-sm transition-colors ${selectedSymptoms.includes(symptom)
