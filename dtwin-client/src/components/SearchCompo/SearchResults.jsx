...
const SearchResults = () => {
  ...
  const totalCalories = nutritionData.reduce((sum, item) => {
    // Calculate calories: Carbs & Protein = 4 cal/g, Fat = 9 cal/g
            ...
            <div className="flex gap-4 px-2 items-center justify-between">
              <div className="relative  flex justify-start w-[8rem]">
                <input
                  type="text"
                  value="100"
                  className="w-[5rem] p-2 border rounded-lg text-center"
                />
                <span className="absolute right-3 top-2.5 text-gray-400">g</span>
              </div>
              <Button className="bg-red-400 hover:bg-red-500 text-white px-6 w-[9rem]">
                Add food
              </Button>
            </div>
