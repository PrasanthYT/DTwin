...
const healthSuggestions = [
  {
    ...
    icons: [
      ...
      <Plus key="2" className="text-white h-5 w-5" />,
    ],
  },
  {
...
const HealthSuggestionCard = ({ title, description, color, icons = [] }) => {
  return (
    <Card className="p-4 md:p-6 hover:shadow-lg transition-shadow duration-200 cursor-pointer bg-white">
      <div className="flex items-center gap-4 md:gap-6 w-full">
        <div className="flex-shrink-0">
          <div className="flex items-center">
            <div className={`${color} p-2.5 rounded-xl z-20 relative`}>
              {icons[0]}
            </div>
            <div className={`${color} p-2.5 rounded-xl z-10 relative -ml-2`}>
              {icons[1]}
            </div>
            <div
              className={`${color} opacity-80 p-2.5 rounded-xl relative -ml-2`}
            >
              {icons[2]}
            </div>
          </div>
      ...
      <div className="bg-[#1e2642]  px-6 md:px-12 pb-32 md:pb-48 rounded-b-[3rem] md:rounded-b-[4rem]">
        ...
        <div className="flex items-center gap-6 pt-8 md:pt-12">
          <Button
            variant="outline"
            size="icon"
            className="h-12 w-12 rounded-xl border-gray-200 bg-white/10 hover:bg-white/20"
          >
