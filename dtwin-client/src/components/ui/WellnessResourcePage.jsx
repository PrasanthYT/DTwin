...
import { ArrowLeft, Search, BookOpen, Video, Clock, Users, ExternalLink, Bookmark, Star, Filter } from 'lucide-react';
    ...
    const fetchResources = async () => {
      try {
        ...
        const transformedData = data.results.map((item, index) => ({
          id: item.id,
          title: item.title.slice(0, 60) + (item.title.length > 60 ? '...' : ''),
          type: ['Course', 'Workshop', 'Video Series'][index % 3],
          duration: ['4 weeks', '6 sessions', '3 hours'][index % 3],
          instructor: ['Dr. Sarah Chen', 'Mike Thompson', 'Dr. Emma Roberts'][index % 3],
          enrolled: Math.floor(Math.random() * 2000) + 500,
          rating: (Math.random() * (5 - 4) + 4).toFixed(1),
          image: `/api/placeholder/${400}/${250}`,
          category: ['Mental Health', 'Nutrition', 'Physical Fitness', 'Meditation'][index % 4],
    ...
    <div className="bg-orange-50 min-h-screen">
      ...
      <div className="bg-gradient-to-br from-orange-400 to-yellow-500 p-4 pb-8 text-white">
        <div className="flex items-center mb-6">
          ...
          <div className="ml-2">
            <p className="text-orange-100 text-sm font-medium tracking-wider uppercase">Wellness Journey</p>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight">Resource Library</h1>
          </div>
        </div>
      ...
      <div className="px-4 md:px-6 -mt-6">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
          {categories.map((category) => (
            <Card key={category.id} className="hover:shadow-lg transition-all duration-300 cursor-pointer bg-white">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {resources.map((resource) => (
              <Card key={resource.id} className="overflow-hidden hover:shadow-lg transition-all duration-300 bg-white">
                ...
                <CardContent className="p-4">
                  ...
                  <p className="text-sm text-gray-600 mb-3">{resource.summary}</p>
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {guide.new && (
                        <span className="bg-yellow-400 text-yellow-800 text-xs px-2 py-0.5 rounded-full">New</span>
                      )}
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-orange-600">{guide.category}</span>
